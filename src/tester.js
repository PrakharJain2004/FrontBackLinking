...
const PostPage = ({ switchToDashboard, users }) => {
    ...
    const fetchUserSuggestions = async (searchQuery) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://172.17.6.223:8000/users/?search=${searchQuery}`, {
                headers: {
                    Authorization: `Token ${token}`,
                },
            });
            ...
            setSuggestedMentionedUsers(userData); // Update suggestions with fetched data
        } catch (error) {
            // Handle errors
        }
    };
    ...
    const fetchAllUsers = async () => {
        try {
            ...
            const userData = response.data;
            setUsers(userData); // Update the existing users state
        } catch (error) {
            // Handle errors
        }
    };
    ...
    const handleInputChange = (event) => {
        ...
        if (inputText.length >= 3 && inputText.includes('@')) {
            ...
            if (mentionInput.length >= 3) {
                setSearchQuery(mentionInput);

                // Filter usernames based on mentionInput from the fetched users
                const filteredSuggestions = users.filter((user) =>
                    user.username.toLowerCase().includes(mentionInput)
                );

                setSuggestedMentionedUsers(filteredSuggestions);
            } else {
